...
export default function WalkMapScreen({ route }) {
  ...
  useEffect(() => {
    try {
      ...
      Toast.show({
        type: 'error',
        text1: 'Erro ao carregar mapa',
        text2: 'Não foi possível inicializar o mapa. Tente novamente.'
      });
    }
  }, [walk]);
    ...
    return (
      <View style={[styles.container, { justifyContent: 'center', alignItems: 'center', padding: spacing.lg }]}>
        ...
        <Text style={[typography.body, { color: colors.textMuted, textAlign: 'center' }]}>
          Não foi possível carregar o mapa. Verifique se o aplicativo tem as permissões necessárias.
        </Text>
        {walk && (
      ...
      <MapView
        ...
        onError={(error) => {
          ...
          Toast.show({
            type: 'error',
            text1: 'Erro no mapa',
            text2: 'Ocorreu um erro ao exibir o mapa. Tente novamente.'
          });
        }}
        onMapReady={() => {
