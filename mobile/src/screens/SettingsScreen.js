        ...
        <Card variant="elevated" style={{marginBottom: spacing.lg}}>
          ...
          <View style={{marginBottom: spacing.lg, padding: spacing.md, backgroundColor: colors.primaryLight + '10', borderRadius: radius.md}}>
            ...
            <Text style={[typography.bodySmall, {color: colors.textMuted, lineHeight: 20}]}>
              O backend permite salvar seus dados em um servidor (H2 Database) ao invés de apenas localmente. 
              Ative o backend e configure a URL da API do servidor.
            </Text>
          </View>
            ...
            <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: spacing.sm}}>
              <View style={{flex: 1}}>
                ...
                <Text style={[typography.bodySmall, {color: colors.textMuted, marginTop: spacing.xs}]}>
                  {useBackend 
                    ? 'Dados serão salvos no servidor (H2 Database)' 
                    : 'Dados serão salvos apenas localmente (AsyncStorage)'}
                </Text>
              </View>
              <TouchableOpacity
            ...
            <View style={{marginBottom: spacing.lg}}>
              ...
              <View style={{marginTop: spacing.sm, gap: spacing.xs}}>
                <Text style={[typography.caption, {color: colors.textMuted, lineHeight: 18}]}>
                  📱 <Text style={{fontWeight: '600'}}>Emulador Android:</Text> http://10.0.2.2:8080
                </Text>
                <Text style={[typography.caption, {color: colors.textMuted, lineHeight: 18}]}>
                  📱 <Text style={{fontWeight: '600'}}>Dispositivo físico / APK:</Text> http://[IP_DO_SERVIDOR]:8080
                </Text>
                <Text style={[typography.caption, {color: colors.textMuted, lineHeight: 18}]}>
                  💡 <Text style={{fontWeight: '600'}}>Dica:</Text> Use o IP local do seu computador (ex: 192.168.1.100:8080)
                </Text>
                <Text style={[typography.caption, {color: colors.warning, lineHeight: 18, marginTop: spacing.xs}]}>
                  ⚠️ <Text style={{fontWeight: '600'}}>Importante:</Text> Em APK (EAS Build), o app funcionará normalmente, mas você precisa usar o IP real do servidor, não 10.0.2.2
                </Text>
              </View>
            </View>
          )}
